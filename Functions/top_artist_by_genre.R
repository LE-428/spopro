...
top_artist_by_genre <- function(data_table_ext, top_x = 10){
  ...
  # Split the genre list (concatenated with ",") into single genres
  data_table_ext <- data_table_ext %>%
    mutate(artist_genres = strsplit(artist_genres, ",")) %>%
    tidyr::unnest(artist_genres) %>%  # every genre as one row
    mutate(artist_genres = str_trim(artist_genres)) # remove blank symbol
 
  # Determine the favorite artist per genre
  top_artists <- data_table_ext %>%
    ...
    slice_max(n, n = 1) %>%  # Take the artists with the most rows per genre
    ungroup() %>% 
    rename(
      Genre = artist_genres,
