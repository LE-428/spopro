...

{
  time_artist_activity <- function(data_frame) {
    data_frame <-
      ...
      ) # Filter out streams shorter than 30s; Spotify counts a stream after 30s
    ...
    matches <-
      as.integer(regmatches(
        data_frame$ts,
        regexpr("(?<=T)\\d{2}", data_frame$ts, perl = TRUE)
      ))
    # print(head(matches))
    activity_hours <-
      data.frame(
        hour = matches,
        master_metadata_album_artist_name = data_frame$master_metadata_album_artist_name,
        track_name = data_frame$master_metadata_track_name
      )
    temp <-
      subset(
        activity_hours,
        hour %in% c(1, 2, 3, 4, 5, 6) &
          master_metadata_album_artist_name == "Frank Ocean"
      )
    print(temp)
    
    # Determine the most frequent artist per hour
    most_frequent_artist <- activity_hours %>%
      group_by(hour) %>%  # Group by hour
      count(master_metadata_album_artist_name) %>%  # Count how often each artist occurs
      arrange(hour, desc(n)) %>%  # Sort by hour and frequency (descending)
      slice(1)
    ...
    # Print the result
    print(result, n = 24) # print() outputs and returns at the same time
    # return(result)
  }
