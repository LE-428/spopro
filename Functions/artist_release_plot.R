...
artist_release_plot <- function(data_frame_ext, artist_string, feature_search_bool = FALSE, exact_search = FALSE) {
  ...
  release_summary <- aux_table %>%
    ...
    summarise(
      ...
      album_name = first(master_metadata_album_album_name)  # use first album title
    ) %>%
    arrange(release_date)
  
  # Cover every day between the first and last release
  ...
  # Group by release date and count number of unique songs
  release_summary_full <- release_summary %>%
    mutate(album_name = replace_na(album_name, ""))  # only release days
  ...
  ggplot() +
    ...
    # Streaming activity as line + dots
