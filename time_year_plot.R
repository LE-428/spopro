...
time_year_plot <- function(data_table){
  all_years <- sort(unique(substr(data_table$ts, 1, 4)), decreasing = FALSE)
  
  listening_times <- numeric(length(all_years))  # Vektor für Hörzeiten initialisieren
  
  for (i in seq_along(all_years)) {
    year <- all_years[i]
    listening_times[i] <- listening_time(extract_year(year, data_table))  # Hörzeit für jedes Jahr speichern
  }
