...
track_duration_plot <- function(data_table_ext, sort_by_streams = TRUE) {
  ...
  if (sort_by_streams == FALSE) {
    ...
    ylabel = "Anzahl der einzigartigen Lieder"
  } else {
    duration_classes <- listened_tracks$duration_ms %/% (30 * 1000)
    ylabel = "Anzahl der Streams"
  }
  ...
  barplot_heights <- barplot(
    ...
    main = "Verteilung der Dauer der Lieder",  # Titel des Plots
    xlab = "Dauer in Sekunden",                            # Beschriftung der X-Achse
